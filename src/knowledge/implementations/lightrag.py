...
class LightRagKB(KnowledgeBase):
    ...
    def _get_llm_func(self, llm_info: dict):
        ...
        force_model_spec = os.getenv("LIGHTRAG_FORCE_LLM_MODEL", "").strip()
        if force_model_spec:
            model_spec = force_model_spec
            logger.info(f"Using forced LightRAG LLM from env: {model_spec}")
        # 如果用户选择了LLM，使用用户选择的；否则使用环境变量默认值
        elif llm_info and llm_info.get("model_spec"):
        ...
        async def llm_model_func(prompt, system_prompt=None, history_messages=[], **kwargs):
            async with self._lightrag_llm_semaphore:
