...
def _format_dt(value: object) -> str | None:
    """Format datetime fields with a cast to avoid ORM typing noise."""
    return format_utc_datetime(cast(dt.datetime | None, value))
...
class User(Base):
    ...
    def to_dict(self, include_password: bool = False) -> dict[str, Any]:
        result = {
            ...
            "created_at": _format_dt(self.created_at),
            "last_login": _format_dt(self.last_login),
            "login_failed_count": self.login_failed_count,
            "last_failed_login": _format_dt(self.last_failed_login),
            "login_locked_until": _format_dt(self.login_locked_until),
            "is_deleted": self.is_deleted,
            "deleted_at": _format_dt(self.deleted_at),
        }
        if include_password:
