"""AgentPlatformFactory — builds agent graphs from context configuration.
...
class AgentPlatformFactory:
    """Factory that builds agent graphs from AgentPlatformContext.
    ...
    def _compute_cache_key(self, context: AgentPlatformContext) -> str:
    ...
    def clear_cache(cls) -> None:
        ...
        logger.info("AgentPlatformFactory: graph cache cleared")

    def _maybe_get_spawn_tool(self, context: AgentPlatformContext) -> list:
        ...
        logger.info(f"AgentPlatformFactory: SpawnTool enabled (max_concurrency={max_conc})")
        return [spawn_tool]

    async def _collect_mcp_tools(self, context: AgentPlatformContext) -> list[Any]:
        ...
        for server_name, result in zip(unique_server_names, results, strict=False):
            if isinstance(result, Exception):
                logger.warning(f"AgentPlatformFactory: failed to load MCP tools from '{server_name}': {result}")
                continue
            all_tools.extend(result)

        elapsed_ms = (time.perf_counter() - started_at) * 1000
        logger.info(
            f"AgentPlatformFactory: loaded {len(all_tools)} MCP tools from "
    ...
    async def build_graph(
        ...
        *,
        ...
        Args:
            context: The AgentPlatformContext with all configuration.
        ...
        """
        runtime_context = runtime_context_service.build_runtime_context(
            {
        ...
        if use_cache:
            ...
            if cached is not None:
                logger.info(f"AgentPlatformFactory: using cached graph (key={cache_key})")
                return cached
    ...
    async def _build_single_agent(
        ...
        *,
        ...
        logger.info("AgentPlatformFactory: built single agent (disabled mode)")
        return graph
    ...
    async def _build_deep_agents_mode(
        ...
        *,
        ...
        logger.info(
            f"AgentPlatformFactory: built Deep Agents mode with "
    ...
    async def _build_supervisor_mode(
        ...
        *,
        ...
        logger.info(
            f"AgentPlatformFactory: built Supervisor mode with "
    ...
    async def _build_swarm_mode(
        ...
        *,
        ...
        logger.info(
            f"AgentPlatformFactory: built Swarm mode with "
