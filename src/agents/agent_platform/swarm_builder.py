"""AgentPlatform swarm builder — handoff-based multi-agent collaboration.
    ...
    *,
    ...
    for config in subagent_configs:
        ...
        for other_config in subagent_configs:
            ...
            other_desc = other_config.get("description", "Specialist agent")
            handoff_tool = create_handoff_tool(
                agent_name=other_name,
                description=f"Transfer conversation to {other_name}: {other_desc}",
            )
        ...
        team_contract = (
            ...
            f"- Allowed handoff targets: "
            f"{', '.join(allowed_targets_list) if allowed_targets_list else 'All other agents'}\n"
        ...
        # Create the agent using create_deep_agent (consistent with supervisor mode)
    ...
    if default_active_agent is None:
        ...
        logger.warning(
            f"SwarmBuilder: default_active_agent '{default_active_agent}' not found, falling back to '{agent_names[0]}'"
        )
        default_active_agent = agent_names[0]
