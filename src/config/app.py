...
# Python 3.11+ uses stdlib tomllib; keep tomli fallback for older runtimes.
try:
...
try:
    ...
    class _TomliWProxy:
        @staticmethod
        def dump(_data, _fp):
            raise ModuleNotFoundError("tomli_w is required when saving config files")

    tomli_w = _TomliWProxy()
...
class Config(BaseModel):
    ...
    default_model: str = Field(
        default="dashscope/qwen-max-latest",
        description="默认对话模型",
    )
