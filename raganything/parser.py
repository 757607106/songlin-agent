...
class MineruAPIParser(Parser):
    """MinerU HTTP API parser that reuses project-level MinerU service integration."""

    def __init__(self) -> None:
        super().__init__()
        self.server_url = os.getenv("MINERU_API_URI") or "http://localhost:30001"
        self.parse_endpoint = f"{self.server_url}/file_parse"

    @staticmethod
    def _to_text_blocks(text: str, page_idx: int = 0) -> List[Dict[str, Any]]:
        ...
        for paragraph in re.split(r"\n\s*\n", content):
            ...
            if not paragraph:
                continue
            blocks.append({"type": "text", "text": paragraph, "page_idx": page_idx})
        return blocks
    ...
    def _build_service_payload(
        self, method: str = "auto", lang: Optional[str] = None, **kwargs
    ) -> Dict[str, Any]:
        ...
        if kwargs.get("start_page") is not None:
            payload["start_page_id"] = kwargs["start_page"]
        if kwargs.get("end_page") is not None:
            payload["end_page_id"] = kwargs["end_page"]
        if kwargs.get("formula") is not None:
            payload["formula_enable"] = bool(kwargs["formula"])
        if kwargs.get("table") is not None:
            payload["table_enable"] = bool(kwargs["table"])

        if payload["backend"] == "vlm-http-client":
            vlm_server = kwargs.get("server_url") or os.getenv("MINERU_VL_SERVER")
            if not vlm_server:
                raise RuntimeError(
                    "MINERU_VL_SERVER is required when backend=vlm-http-client"
                )
            payload["server_url"] = vlm_server

        return payload

    def _request_mineru_zip(
        self, input_path: Path, method: str = "auto", lang: Optional[str] = None, **kwargs
    ) -> bytes:
        import requests
        ...
        payload = self._build_service_payload(method=method, lang=lang, **kwargs)
        timeout_seconds = int(os.getenv("MINERU_TIMEOUT", "1800"))
        ...
        if response.status_code != 200:
            ...
            try:
                ...
                pass
            raise RuntimeError(
                f"MinerU API request failed with status {response.status_code}: {detail}"
            )

        return response.content

    def _prepare_output_dir(
        self, file_path: Path, output_dir: Optional[str] = None
    ) -> Path:
        if output_dir:
            base_output_dir = self._unique_output_dir(output_dir, file_path)
        else:
            base_output_dir = file_path.parent / "mineru_api_output"
        base_output_dir.mkdir(parents=True, exist_ok=True)

        file_output_dir = base_output_dir / file_path.stem / "mineru_api"
        file_output_dir.mkdir(parents=True, exist_ok=True)
        return file_output_dir

    def _extract_mineru_zip(
        self, zip_bytes: bytes, output_dir: Path
        ...
        markdown_content = self._to_markdown(markdown_file)
        ...
        for image_path in output_dir.rglob("*"):
            ...
            image_map[relative] = absolute
            image_map[f"./{relative}"] = absolute
            image_map[f"/{relative}"] = absolute
            image_map[image_path.name] = absolute
    ...
    def _resolve_image_path(
        self, raw_path: str, image_map: Dict[str, str], output_dir: Path
    ) -> str:
        clean_path = raw_path.strip().strip('"').strip("'")
        if clean_path in image_map:
            return image_map[clean_path]

        from pathlib import PurePosixPath

        normalized = PurePosixPath(clean_path).as_posix()
    ...
    def _markdown_to_content_list(
        self, markdown: str, image_map: Dict[str, str], output_dir: Path
    ) -> List[Dict[str, Any]]:
        import re
        ...
        def flush_text() -> None:
            ...
            if not text:
                return
            content_list.extend(self._to_text_blocks(text, page_idx=page_idx))
        ...
        while idx < len(lines):
            ...
            # Try to infer page boundaries from common markdown markers.
            if re.match(r"^#{1,6}\s*page\s+\d+", stripped, flags=re.IGNORECASE):
                page_match = re.search(r"(\d+)", stripped)
                if page_match:
                    page_idx = max(0, int(page_match.group(1)) - 1)
            ...
            if stripped.startswith("$$"):
                ...
                if stripped != "$$" and stripped.endswith("$$") and len(stripped) > 4:
                    equation_lines.append(stripped[2:-2].strip())
                    idx += 1
                else:
                    if stripped != "$$":
                        equation_lines.append(stripped[2:].strip())
                    idx += 1
                    while idx < len(lines):
                        eq_line = lines[idx]
                        if eq_line.strip().endswith("$$"):
                            tail = eq_line.strip()
                            if tail != "$$":
                                equation_lines.append(tail[:-2].strip())
                            idx += 1
                            break
                        equation_lines.append(eq_line)
                        idx += 1

                latex = "\n".join(part for part in equation_lines if part).strip()
                if latex:
                    content_list.append(
                        {
                            "type": "equation",
            ...
            # Markdown tables: header line + separator line
            if (
                "|" in stripped
                and idx + 1 < len(lines)
                and self._is_table_separator(lines[idx + 1].strip())
            ):
                flush_text()
            ...
            if image_matches:
                flush_text()
                residual_text = image_pattern.sub("", line).strip()
                for match in image_matches:
                    alt_text = (match.group(1) or "").strip()
                    raw_path = (match.group(2) or "").strip()
                    resolved_path = self._resolve_image_path(raw_path, image_map, output_dir)
                    content_list.append(
                        {
                            "type": "image",
                ...
                if residual_text:
                    text_buffer.append(residual_text)
                idx += 1
                continue
    ...
    def parse_pdf(
        ...
        **kwargs,
        ...
        file_output_dir = self._prepare_output_dir(pdf_path, output_dir)
        zip_bytes = self._request_mineru_zip(pdf_path, method=method, lang=lang, **kwargs)
        markdown, image_map = self._extract_mineru_zip(zip_bytes, file_output_dir)
        content_list = self._markdown_to_content_list(markdown, image_map, file_output_dir)

        if not content_list:
            return self._to_text_blocks(markdown)
        return content_list
    ...
    def parse_image(
        ...
        **kwargs,
        ...
        file_output_dir = self._prepare_output_dir(image_path, output_dir)
        zip_bytes = self._request_mineru_zip(image_path, method="ocr", lang=lang, **kwargs)
        markdown, image_map = self._extract_mineru_zip(zip_bytes, file_output_dir)
        content_list = self._markdown_to_content_list(markdown, image_map, file_output_dir)
    ...
    def parse_text_file(
        ...
        **kwargs,
        ...
        markdown = self._to_markdown(text_path)
        file_output_dir = self._prepare_output_dir(text_path, output_dir)
        content_list = self._markdown_to_content_list(markdown, {}, file_output_dir)
        if not content_list:
            return self._to_text_blocks(markdown)
        return content_list
    ...
    def parse_document(
        ...
        **kwargs,
        ...
        if ext in self.TEXT_FORMATS:
            return self.parse_text_file(file_path, output_dir=output_dir, lang=lang, **kwargs)
        if ext in {".html", ".htm", ".xhtml"}:
            return self.parse_text_file(file_path, output_dir=output_dir, lang=lang, **kwargs)

        raise ValueError(
            f"Unsupported file format: {ext}. "
            "mineru_api parser supports PDF, image, office, text, and HTML formats."
        )
    ...
    def check_installation(self) -> bool:
        try:
            ...
            if response.status_code != 200:
                return False
            data = response.json()
            return "/file_parse" in data.get("paths", {})
        except Exception:
            return False
...
SUPPORTED_PARSERS = ("mineru", "mineru_api", "docling", "paddleocr")
